...
```{r prostate_cancer_top_10_nostop}
dataset %>%
  filter(term == "prostate cancer") %>%
  unnest_tokens(token, abstract, token = "words") %>%
  anti_join(stop_words, by = c("token" = "word")) %>%
...
```{r cystic_fibrosis_top_10_nostop}
dataset %>%
  filter(term == "cystic fibrosis") %>%
  unnest_tokens(token, abstract, token = "words") %>%
  anti_join(stop_words, by = c("token" = "word")) %>%
...
For term "meningistis", the top 5 words are "patients" "meningistis""meningeal""csf""clinical"
